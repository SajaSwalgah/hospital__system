...
$firstName = $lastName = $username = $email = $phone = $gender = $country = $password = $confPassword = $addPatient = '';
if (isset($_POST['apply'])) {
    ...
    if (!array_filter($errors)) {
        $sql = "SELECT * FROM patients WHERE username='$username' OR email='$email'";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) == 1) {
            $addPatient = "This Account exist";
        } else {
        $query = "INSERT INTO patients(firstName, lastName, username, email, gender, phone, country, password, date_reg, profile)
            VALUES('$firstName', '$lastName', '$username', '$email', '$gender', '$phone', '$country', '$password', NOW(), 'patient.jpg')";
        $result2 = mysqli_query($conn, $query);
        if ($result2) {
            $addPatient = "You Have Successfully Registered";
            header('Location:patient-login.php');
        } else {
            $addPatient = "Something Went Wrong!";
        }
        }
    }
}
            ...
            <div class="row">
                ...
                <div class="col-md-6 jumbotron">
                    ...
                    <form action="register.php" method="POST" class="my-2">
                        ...
                        <input type="submit" name="apply" class="btn btn-success" value="Register">
                        <p>You already have an account?? <a href="patient-login.php">Login In</a></p>
                    </form>
                    <?php if ($addPatient) : ?>
                        <div class='alert alert-danger center'><?php echo $addPatient ?></div>
                    <?php else : ?>
                        <div></div>
                    <?php endif ?>
                </div>
